...
const GoogleInfoModal = ({ googleUser, onComplete, loading }) => {
    ...
    const onSubmit = async (data) => {
        setUploading(true);
        try {
            // Upload child image to imgbb
            let childImageURL = '';
            if (data.childImage?.[0]) {
                childImageURL = await imageUpload(data.childImage[0]);
            }
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 overflow-y-auto">
            <motion.div
                ...
                className="bg-base-100 rounded-3xl w-full max-w-md p-8 border border-base-300 shadow-2xl my-8"
            >
                {/* Header */}
                <div className="text-center mb-6">
                    <div className="text-4xl mb-3">👋</div>
                    <h2 className="text-2xl font-bold text-neutral">{c.title}</h2>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <label htmlFor="googleChildImg"
                            className={`flex flex-col items-center justify-center h-24 rounded-2xl border-2 border-dashed cursor-pointer transition-all hover:border-primary/50 hover:bg-primary/5 ${errors.childImage ? 'border-error' : 'border-base-300'}`}>
                            {childPreview
